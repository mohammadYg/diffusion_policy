...
class TrainPacDiffusionUnetLowdimWorkspace(BaseWorkspace):
    ...
    def _train_prior(self, cfg, prior_policy: PacDiffusionUnetLowdimPolicy, prior_dataset, device):
        """
        ...
        with tqdm.tqdm(total=prior_num_updates, desc="Training data-dependent prior",
            ...
            while step < prior_num_updates:
                for batch in prior_dataloader:
                    ...
                    raw_loss = prior_policy.compute_loss(batch, stochastic=True, train=True)
                    raw_loss.backward()
    ...
    @staticmethod
    def _resolve_prior_checkpoint_path(cfg, n_prior_demos: int):
        """
    ...
    def _load_cached_prior(self, cfg, prior_ckpt_path, normalizer, device):
        """
        ...
        prior_policy: PacDiffusionUnetLowdimPolicy = hydra.utils.instantiate(cfg.policy)
    ...
    def _get_trained_prior(self, cfg, prior_ckpt_path, prior_dataset, n_prior_demos, normalizer, device) -> PacDiffusionUnetLowdimPolicy:
        """
    ...
    def _setup_train_dataset(self, cfg, dataset, normalizer, device):
        ...
        the posterior split, and n_bound (see run()) is derived from this
        returned dataset's size - so it always correctly reflects the disjoint
        posterior/bound split rather than the full (prior+posterior) dataset. On
        a fresh start (global_step == 0), self.model/self.ema_model get seeded
        (prior AND posterior) from the trained (or cached) prior via
        prior_initialization(). On resume, self.model's posterior already
        reflects partially completed training and must not be reset - but we
        still refresh the FIXED prior (only, via init_posterior=False) from the
        cached prior checkpoint if one is available, so the PAC-Bayes bound
        keeps measuring KL against the intended data-dependent prior. We
        deliberately never *train* a prior on resume: training is stochastic,
        so a freshly retrained prior would differ from the one the checkpointed
    ...
    def run(self):
        ...
        # configure dataset for covariance_spectrum (always the FULL dataset -
        # this only estimates data statistics for the NLL integration range,
        # unrelated to the PAC-Bayes bound's prior/posterior data split below)
        ...
        # trains on - see _setup_train_dataset()/_get_trained_prior(). n_bound
        # (used by compute_bound in the training loop) is derived from THIS
        # returned dataset's size, so it always reflects the disjoint
        ...
        with JsonLogger(log_path) as json_logger:
            ...
            # Correctly reflects only the posterior/bound split's size when a
            # data-dependent prior is active - see _setup_train_dataset().
